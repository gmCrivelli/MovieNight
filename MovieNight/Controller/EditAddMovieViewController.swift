...
class EditAddMovieViewController: UIViewController {
    ...
    /// Configure the view using the given movie, if it exists. If there is a movie, display as editing screen. If there's no movie, display a registering screen to create one.
    ///
    ...
    @IBAction func addPosterImage(_ sender: Any) {
        let alert = UIAlertController(title: "Pôster do Filme", message: "Escolha de onde recuperar o pôster", preferredStyle: .actionSheet)
    ...
    @IBAction func deleteMovie(_ sender: Any) {
        ...
        let alert = UIAlertController(title: "Atenção!", message: "Tem certeza de que deseja deletar este filme? Esta ação não pode ser desfeita.", preferredStyle: UIAlertController.Style.alert)
